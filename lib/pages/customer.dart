...
import 'QRScanPage.dart'; // تأكد من استيراد الصفحة الصحيحة
...
class _CustomerState extends State<Customer> {
  ...
  bool isQRMatched = false; // للتحكم في إظهار زر الدفع
  bool isCustomerLocked = false; // لمنع تغيير العميل بعد المسح

  void _navigateToQRScanPage() async {
    ...
    if (qrData != null) {
      // تحقق من البيانات التي تم مسحها
      _processQRData(qrData);
    }
  }

  void _processQRData(String qrData) async {
    // تحويل البيانات إلى رقم ID
    int? customerId = int.tryParse(qrData);
    ...
    // جلب بيانات العميل باستخدام الـ ID
    var customerData = await Contact().getCustomerDetailById(customerId);
    ...
    if (customerData != null) {
      setState(() {
        ...
        isQRMatched = (customerId == selectedCustomer['id']);
        isCustomerLocked = true; // قفل العميل بعد نجاح المسح
        if (isQRMatched) {
          _playConfirmationSound();  // تشغيل الصوت عند المطابقة
        }
      });
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _playConfirmationSound() async {
    await _audioPlayer.play(AssetSource('sounds/confirm.mp3'));
  }

  @override
  void dispose() {
    _audioPlayer.dispose(); // تنظيف الذاكرة عند إغلاق الصفحة
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          ...
          children: [
            Visibility(
              visible: !isQRMatched, // إخفاء زر مسح QR عند التطابق
              child: TextButton(
            ...
            Visibility(
              visible: isQRMatched && argument!['is_quotation'] == null,
              child: TextButton(
            ...
            Visibility(
              visible: isQRMatched, // عرض زر الدفع فقط عند تطابق QR
              child: cartBottomBar(
                '/checkout',
  ...
  Widget customerList() {
    return SearchChoices.single(
      ...
      onChanged: (newValue) {
        if (!isCustomerLocked) { // منع التغيير إذا كان العميل مغلقًا
          setState(() {
            selectedCustomer = jsonDecode(newValue);
          });
        } else {
          // إظهار رسالة للمستخدم بأنه لا يمكن تغيير العميل
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('لا يمكن تغيير العميل بعد المسح الناجح')),
          );
        }
      },
      isExpanded: true,
